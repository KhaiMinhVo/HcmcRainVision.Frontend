...
/** POST api/Weather/test-ai ù upload image for AI prediction test (multipart). */
...
export function mapLatestToRainPoint(item: WeatherLatestItemDto): RainDataPoint {
  ...
  return {
    ...
    lng: item.Longitude,
    rainLevel,
    timestamp: item.Timestamp || item.TimeAgo, // ?u tiÍn ISO timestamp, fallback sang TimeAgo
  };
}
